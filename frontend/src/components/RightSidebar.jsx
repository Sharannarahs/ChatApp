...
const RightSidebar = () => {
  ...
  useEffect(()=>{
    setMsgImages(
        messages.filter(msg => msg.image).map(msg => msg.image) // filters images from messages and then maps only image names
    )

  },[messages])

  return selectedUser && (
    <div className={`bg-black/60 text-white w-full relative overflow-y-scroll 
